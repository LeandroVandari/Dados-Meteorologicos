...
def baixar(arquivo: Path | None = None):
    ...
    sala_de_situacao = requests.get(
        "https://saladesituacao.rs.gov.br/api/station/ana"
    ).json()
    hidroweb_convencional = requests.get(
        "https://www.snirh.gov.br/hidroweb/rest/api/dadosHistoricos?estadoCodigo=24&tipoEstacao=F&size=10000&page=0&operando=1",
        headers=headers,
    ).json()
